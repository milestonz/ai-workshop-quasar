...
app.post('/api/convert-slides', (req, res) => {
    ...
    (0, child_process_1.exec)(buildCommand, { cwd: process.cwd() }, (error, stdout, stderr) => {
        if (error) {
        ...
        // 변환된 HTML 파일들 확인
        const generatedDir = path_1.default.join(process.cwd(), 'public', 'generated', 'slides');
        let convertedFiles = [];
        try {
            if (fs_1.default.existsSync(generatedDir)) {
                convertedFiles = fs_1.default
...
app.post('/api/build-slides', (req, res) => {
    ...
    (0, child_process_1.exec)(buildCommand, { cwd: process.cwd() }, (error, stdout, stderr) => {
        ...
        // 빌드된 HTML 파일들 확인
        const generatedDir = path_1.default.join(process.cwd(), outputDir);
        let builtFiles = [];
        try {
            if (fs_1.default.existsSync(generatedDir)) {
                builtFiles = fs_1.default
                    .readdirSync(generatedDir)
                    .filter((file) => file.endsWith('.html'))
                    .map((file) => file.replace('.html', ''));
            }
...
app.post('/api/email/learning-completion', async (req, res) => {
    try {
        ...
        // 이메일 서비스가 설정되지 않은 경우
        if (!emailService_1.emailService.isInitialized()) {
            return res.json({
                success: false,
                message: '이메일 서비스가 설정되지 않았습니다. 관리자에게 문의하세요.',
            });
        }
        await emailService_1.emailService.sendLearningCompletionEmail(recipientEmail, studentName, courseName, completionDate);
        res.json({
            success: true,
...
app.post('/api/email/course-share', async (req, res) => {
    try {
        ...
        await emailService_1.emailService.sendCourseShareEmail(recipientEmail, '수강생', senderName, courseName, shareUrl);
        res.json({
            success: true,
...
app.post('/api/email/system-notification', async (req, res) => {
    try {
        ...
        await emailService_1.emailService.sendSystemNotificationEmail(recipientEmail, subject, message);
        res.json({
            success: true,
...
app.get('/api/email/status', (req, res) => {
    res.json({
        success: true,
        configured: emailService_1.emailService.isInitialized(),
        message: emailService_1.emailService.isInitialized()
            ? '이메일 서비스가 설정되어 있습니다.'
...
app.post('/api/survey/submit', (req, res) => {
    try {
        ...
        // 설문조사 데이터를 파일로 저장
        const surveyFile = path_1.default.join(process.cwd(), 'data', 'surveys.json');
        const surveyDir = path_1.default.dirname(surveyFile);
        // 디렉토리가 없으면 생성
        if (!fs_1.default.existsSync(surveyDir)) {
            fs_1.default.mkdirSync(surveyDir, { recursive: true });
        }
        // 기존 설문 데이터 읽기
        let surveys = [];
        if (fs_1.default.existsSync(surveyFile)) {
            try {
                const existingData = fs_1.default.readFileSync(surveyFile, 'utf8');
        ...
        // 파일에 저장
        fs_1.default.writeFileSync(surveyFile, JSON.stringify(surveys, null, 2));
        res.json({
            success: true,
...
app.get('/api/survey/results', (req, res) => {
    try {
        ...
        const surveyData = fs_1.default.readFileSync(surveyFile, 'utf8');
...
const initializeEmailService = () => {
    ...
    if (emailConfig.auth.user && emailConfig.auth.pass) {
        emailService_1.emailService.initialize(emailConfig);
